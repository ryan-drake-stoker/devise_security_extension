    ...
    module PasswordArchivable
      ...
      def password_archive_included?
        ...
        if self.class.deny_old_passwords > 0 and not self.password.nil?
          old_passwords_including_cur_change = self.old_passwords.order_by([:_id, :desc]).limit(self.class.deny_old_passwords)
      ...
      def archive_password
        if self.encrypted_password_changed?
          if self.class.password_archiving_count.to_i > 0
            ...
            self.old_passwords.order_by([:_id, :desc]).skip(self.class.password_archiving_count).each{|p| p.destroy}
          else
